param(
    [string]$OutputDir = "docker/config/loki/tls",
    [string[]]$ServerDnsNames = @("localhost", "loki-gateway", "loki"),
    [string[]]$ServerIpAddresses = @("127.0.0.1")
)

$ErrorActionPreference = "Stop"

# Generate Loki mTLS assets for Promtail clients and the Loki gateway.
...
if ([string]::IsNullOrWhiteSpace($scriptRoot)) {
    throw "Unable to resolve the generate-loki-tls-certs.ps1 script directory."
}
