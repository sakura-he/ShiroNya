...
import { AppExtendedPrismaClient } from './extended-client';
import { MenuStatusEnum, RbacStatus } from './generated/client';

@Injectable()
export class AppPrismaService extends AppExtendedPrismaClient implements OnModuleInit, OnModuleDestroy {
    private readonly logger = createRuntimeLogger(AppPrismaService.name);
    private readonly maxRetries = 5;
    private readonly retryDelay = 3000; // 3秒

    /** 模块初始化时建立 Prisma 连接。 */
    async onModuleInit() {
        this.logger.info.title('AppPrismaService onModuleInit 开始');
        await this.connectWithRetry();
        await this.warmupPool();
        await this.warmupRbacQueryPaths();
        this.logger.info.title('AppPrismaService onModuleInit 完成');
    }
