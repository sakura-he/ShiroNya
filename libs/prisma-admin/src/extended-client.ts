...
function readRequiredEnv(name: string): string {
    ...
    if (!value) {
        throw new Error(`ADMIN_DATABASE_URL 未配置，无法创建 Prisma 连接`);
    }
    return value;
}
...
class UntypedAdminExtendedPrismaClient extends PrismaClient {
    /**
    ...
    constructor(options?: Omit<ConstructorParameters<typeof PrismaClient>[0], 'adapter' | 'accelerateUrl'>) {
        const pool = new Pool({
            connectionString: readRequiredEnv('ADMIN_DATABASE_URL'),
...
const AdminExtendedPrismaClient = UntypedAdminExtendedPrismaClient as unknown as new (
    options?: Omit<ConstructorParameters<typeof PrismaClient>[0], 'adapter' | 'accelerateUrl'>
) => ReturnType<typeof extendClient>;

export { AdminExtendedPrismaClient, AdminExtendedPrismaClient as ExtendedPrismaClient };
