...
import { APP_SPICEDB_BASE_RELATION_PROJECTION_CONSUMER_GROUP } from '../spicedb-projection/spicedb-projection.constants';
import {
    APP_SPICEDB_WATCH_DLQ_TOPIC,
    APP_SPICEDB_WATCH_EVENTS_TOPIC,
    createSpiceDbKafka
...
export class AdminSpiceDbKafkaProjectionConsumerService implements OnApplicationBootstrap, OnModuleDestroy {
    ...
    constructor(
        ...
        this.enabled = this.configService.get<string>('APP_SPICEDB_KAFKA_ENABLED', 'false') === 'true';
        this.eventsTopic = this.configService.get<string>(
            'APP_SPICEDB_KAFKA_WATCH_EVENTS_TOPIC',
            APP_SPICEDB_WATCH_EVENTS_TOPIC
        );
        this.dlqTopic = this.configService.get<string>(
            'APP_SPICEDB_KAFKA_WATCH_DLQ_TOPIC',
            APP_SPICEDB_WATCH_DLQ_TOPIC
        );
        this.consumerGroup = this.configService.get<string>(
            'APP_SPICEDB_KAFKA_PROJECTION_CONSUMER_GROUP',
            APP_SPICEDB_BASE_RELATION_PROJECTION_CONSUMER_GROUP
        );
        this.maxFailuresBeforePause = Number(
            this.configService.get<string>('APP_SPICEDB_KAFKA_PROJECTION_CONSUMER_MAX_FAILURES', '5')
        );
