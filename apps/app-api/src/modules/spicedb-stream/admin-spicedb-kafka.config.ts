...
export const APP_SPICEDB_WATCH_EVENTS_TOPIC = 'app-api.spicedb.watch-events.v1';
export const APP_SPICEDB_WATCH_DLQ_TOPIC = 'app-api.spicedb.watch-dlq.v1';
export const APP_SPICEDB_KAFKA_CLIENT_ID = 'app-api-spicedb-projection';
...
export function getSpiceDbKafkaBrokersOrThrow(configService: ConfigService): string[] {
    const brokers = parseCommaSeparatedEnv(configService.get<string>('APP_SPICEDB_KAFKA_BROKERS'));
    if (brokers.length === 0) {
        throw new Error('缺少 APP_SPICEDB_KAFKA_BROKERS 配置，无法启动 SpiceDB Kafka 投影同步');
    }
...
export function createSpiceDbKafkaSaslOptions(configService: ConfigService): SASLOptions | undefined {
    const username = configService.get<string>('APP_SPICEDB_KAFKA_SASL_USERNAME');
    const password = configService.get<string>('APP_SPICEDB_KAFKA_SASL_PASSWORD');

    if (!username && !password) {
        return undefined;
    }
    if (!username || !password) {
        throw new Error('APP_SPICEDB_KAFKA_SASL_USERNAME 和 APP_SPICEDB_KAFKA_SASL_PASSWORD 必须同时配置');
    }
...
export function createSpiceDbKafkaConfig(configService: ConfigService): KafkaConfig {
    return {
        clientId: configService.get<string>('APP_SPICEDB_KAFKA_CLIENT_ID', APP_SPICEDB_KAFKA_CLIENT_ID),
        brokers: getSpiceDbKafkaBrokersOrThrow(configService),
        ssl: configService.get<string>('APP_SPICEDB_KAFKA_SSL', 'false') === 'true',
