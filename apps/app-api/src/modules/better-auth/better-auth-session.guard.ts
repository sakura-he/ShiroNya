...
export class BetterAuthSessionGuard implements CanActivate {
    private readonly tracer = trace.getTracer('admin-api.better_auth.session');
        ...
        return await this.tracer.startActiveSpan(
            ...
            async (span) => {
                try {
                    ...
                    // Better Auth 在 app 后端只负责身份与 session 生命周期，不读取其角色/组织权限 metadata 做业务授权。
                    return true;
                } catch (error) {
