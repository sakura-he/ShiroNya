...
const REQUEST_COMPOSITE_VERSION_CACHE_KEY = Symbol.for('admin-api.app-api.user-state.composite-version-cache');
...
export class AdminUserStateService implements UserStateHeaderWriter {
    ...
    async getMenuStateVersion(): Promise<string> {
        const cached = await this.getFromCache(APP_API_MENU_VERSION_REDIS_KEY);
        if (cached) return cached;

        // 菜单全局版本采用单记录（type=menu, name=app_api_global），未命中时再初始化
        const record = await this.prismaService.stateVersion.findUnique({
            where: { type_name: { type: StateVersionType.menu, name: APP_API_MENU_VERSION_DB_NAME } },
            select: { version: true }
        });
        if (record?.version) {
            await this.setToCache(APP_API_MENU_VERSION_REDIS_KEY, record.version);
            return record.version;
        }
        ...
        await this.prismaService.stateVersion.create({
            data: {
                ...
                name: APP_API_MENU_VERSION_DB_NAME,
                version: nextVersion
            }
        });
        await this.setToCache(APP_API_MENU_VERSION_REDIS_KEY, nextVersion);
        return nextVersion;
    }
    ...
    private buildUserKey(userId: string): string {
        return createAppApiUserVersionRedisKey(userId);
    }

    /** 组装角色版本缓存键 */
    private buildRoleKey(roleId: number): string {
        return createAppApiRoleVersionRedisKey(roleId);
    }
