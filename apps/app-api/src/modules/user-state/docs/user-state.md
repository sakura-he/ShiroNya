...
- `apps/app-api/src/modules/user-state/admin-user-state.module.ts`
- `apps/app-api/src/modules/user-state/admin-user-state.service.ts`
- `apps/app-api/src/modules/user-state/admin-user-state.service.spec.ts`
...
- `apps/app-api/src/modules/authz-permission/authz-permission.service.ts`
- `apps/app-api/src/modules/system/roles/roles.service.ts`
- `apps/app-api/src/modules/system/menus/menus.service.ts`
- `apps/app-api/src/modules/system/users/users.service.ts`
- `apps/app-api/src/modules/system/user-groups/user-groups.service.ts`

## 2. 模块定位

`AdminUserStateModule` 是 `app-api` 管理侧的“版本感知中枢”。它不直接参与角色、菜单、用户的业务写入，而是把这些业务变更统一翻译成用户状态版本号，再通过响应头把“需要刷新”的信号传给前端。
...
- 环境变量：
    - `APP_REDIS_URL`，或 `APP_REDIS_HOST` / `APP_REDIS_PORT` / `APP_REDIS_USER` / `APP_REDIS_PASSWORD`
    - `APP_DATABASE_URL`
...
- `getRoleStateVersion(roleId, roleName)`：优先读 Redis；未命中时回源 `state_version(type=role, name='app-api:role:<roleId>')`。
- `bumpRoleStateVersion(roleId, roleName)`：同步更新 `app-api:role:<roleId>` DB 记录与 Redis 缓存。
- `getMenuStateVersion()`：优先读 Redis；未命中时回源 `state_version(type=menu, name='app_api_global')`。
...
- 用户版本：`ver:app-api:user:{userId}`
- 角色版本：`ver:app-api:role:{roleId}`
- 菜单全局版本：`ver:app-api:menu:global`

这套键统一以 `ver:app-api:` 开头，用于和其它应用的状态版本键空间隔离。
综合版本本身只在请求内 memo；跨请求缓存由 `/account/navigation` 使用的 `app-api:account:navigation:{userId}:{version}` 承担，版本变化会天然切到新的导航缓存 key。

### 7.2 DB 持久化规则

- 角色版本：`type=role, name='app-api:role:<roleId>'`
- 菜单版本：`type=menu, name='app_api_global'`

其中 `app_api_global` 是 app-api 管理侧菜单专用命名，避免和其它应用的菜单版本记录互相覆盖。
角色版本使用 `type=role, name='app-api:role:<roleId>'` 作为稳定持久化键。
...
测试文件：`apps/app-api/src/modules/user-state/admin-user-state.service.spec.ts`
...
- 示例测试
    ...
    - 菜单版本使用 `app_api_global`
    - 无会话请求跳过版本头写入
    - 同一 request 内重复写版本头只计算一次综合版本，不写 `ver:app-api:composite:{userId}`
...
- 上游用户模块：`../../system/users/docs/user-spicedb-role-relations.md`
