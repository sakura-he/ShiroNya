...
export class ProjectionHealthGateService {
    ...
    async getProjectionHealthGate(): Promise<ProjectionHealthGateResult> {
        if (!this.getBooleanConfig('APP_SPICEDB_PROJECTION_HEALTH_GATE_ENABLED', true)) {
            return {
                usable: true,
                reason: null,
                lag: 0,
        ...
        const driftCount = totalStats
            ...
            : null;
        const lagWarn = this.getNumberConfig('APP_SPICEDB_PROJECTION_HEALTH_LAG_WARN', 1000);
        const dlqWarn = this.getNumberConfig('APP_SPICEDB_PROJECTION_HEALTH_DLQ_WARN', 1);
        const driftWarn = this.getNumberConfig('APP_SPICEDB_PROJECTION_HEALTH_DRIFT_WARN', 1);

        if (!lastRun) {
            return { usable: false, reason: 'projection_reconcile_snapshot_missing', lag, driftCount, unhandledDlqCount };
        }
