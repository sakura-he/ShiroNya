...
describe('ProjectionAuthzReadService', () => {
    beforeEach(() => {
        ...
        mockPrismaService.rbacRole.findMany.mockResolvedValue([{ id: 2, code: 'ops', isSuperAdmin: false }]);
        mockPrismaService.authzResourceRoleBinding.count.mockResolvedValue(0);
        mockPrismaService.authzResourceRoleBinding.findMany.mockResolvedValue([]);
    ...
    it('Redis 脏缓存应删除并回源投影读模型', async () => {
        ...
        expect(mockRedis.setEx).toHaveBeenCalledWith(expect.stringContaining('visible-task-ids'), 60, JSON.stringify([1, 2]));
    });
    ...
    it('应通过 creator 和对象例外关系计算 task 按钮权限', async () => {
        ...
        await expect(service.checkTaskPermissionsFromReadModel([1, 2], ['view', 'update', 'run'], 'user_1')).resolves.toEqual([
            {
                taskId: 1,
                permissions: {
                    view: true,
    ...
    it('task manager 权限命中时应批量放行对应 task 权限', async () => {
        mockPrismaService.authzResourceRoleBinding.findMany.mockResolvedValue([{ resourceId: 'updater' }]);
