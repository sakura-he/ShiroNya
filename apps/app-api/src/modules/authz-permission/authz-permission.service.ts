...
import type { RbacRole } from '@app/prisma-app/generated/client';
...
export class AuthzPermissionService {
    ...
    async renameResource(data: RenameAuthzPermissionResourceType, actorId: string) {
        ...
        await this.prismaService.authzResourceMetadata.upsert({
            ...
            create: {
                resourceType,
                displayName,
                authorizationEnabled: authorizationEnabled ?? this.inferInitialAuthorizationEnabled(definition)
            },
            update: {
                displayName,
        ...
        if (missingResourceTypes.length > 0) {
            await this.prismaService.authzResourceMetadata.createMany({
                data: missingResourceTypes.map((resourceType) => ({
                    ...
                    authorizationEnabled: this.inferInitialAuthorizationEnabled(
    ...
    /**
     * 实体首次入库时，根据 schema 结构生成授权开关初始值。
     */
    private inferInitialAuthorizationEnabled(definition: SpiceDbSchemaDefinitionType) {
        return this.isSchemaDefinitionAuthorizationShapeSupported(definition);
    }
