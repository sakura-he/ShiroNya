...
import { FileMergeStatus, FileChunk } from '@app/prisma-app/generated/client';
...
export class UploadService {
    ...
    async mergeFileChunks(userId: string, fileUID: string) {
        ...
        if (existedMergedFile) {
            ...
            await this.clearMergeChunks(userId, fileUID);
        ...
        const mergedFileName = `${uploadDateTime.format('YYYYMMDDHHmmssSSS')}-${randomUUID()}-${sanitizeStoredFileName(lastChunk.fileName)}`;
        ...
        mergeStream.on('finish', async () => {
            try {
                ...
                await this.clearMergeChunks(userId, fileInfo.fileUid);
            } catch (error) {
                // 保留完整调用：error 级别且 context 含 error 字段，需要落盘 error.stack 进 RuntimeLogEntry.error
     ...
     * 清理合并完成后对应的切片记录与磁盘文件。
     */
    async clearMergeChunks(userId: string, fileUID: string) {
