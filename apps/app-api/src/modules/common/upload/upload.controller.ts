...
import { basename, join as pathJoin, relative } from 'node:path';
import type { BetterAuthSession } from '../../better-auth/better-auth-session.type';
import { UploadService } from './upload.service';

const MAX_UPLOAD_FILE_SIZE_BYTES = 1024 * 1024 * 999;
...
const FileInterceptorOptions = (filePath: string) => ({
    storage: diskStorage({
        ...
        filename(_req, file, cb) {
            const filename = `${file.fieldname}-${dayjs().format('YYYYMMDDHHmmssSSS')}-${randomUUID()}-${sanitizeUploadFileName(file.originalname)}`;
            cb(null, filename);
        }
    })
});
