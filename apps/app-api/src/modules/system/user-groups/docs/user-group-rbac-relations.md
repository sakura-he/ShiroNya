...
- `apps/app-api/src/modules/system/user-groups/user-groups.controller.ts`
- `apps/app-api/src/modules/system/user-groups/user-groups.service.ts`
- `apps/app-api/src/modules/system/user-groups/dto/user-group.dto.ts`
...
当前仓库没有 app-api 专用的用户组管理前端页面。本文只描述 app 侧后端接口；若控制面需要暴露对应 UI，应通过明确的跨应用管理链路接入。
