...
describe('SystemDictsService', () => {
    ...
    it('创建字典时，应要求分类表已存在对应分类', async () => {
        const dict = createDictRecord();
        mockPrismaService.dictCategory.findUnique.mockResolvedValue({ id: 1, name: 'system' });
        ...
        expect(mockPrismaService.dictCategory.findUnique).toHaveBeenCalledWith({
            where: {
                name: 'system'
            }
        });
        expect(mockPrismaService.dictCategory.upsert).not.toHaveBeenCalled();
        expect(mockPrismaService.dict.create).toHaveBeenCalled();
    });
