...
export class SystemDictsService {
    /**
     * 注入 Prisma 服务，用于读写 app_dict 与 dict_key。
     */
    constructor(private readonly prisma: PrismaService) {}

    /**
     * 查询显式字典分类选项；读路径只读取分类表。
     */
    async getDictCategoryOptions(query: QueryDictCategoryOptionsDto) {
        const keywordWhere = query.keyword
            ...
            : undefined;
        const categories = await this.prisma.dictCategory.findMany({
            where: keywordWhere,
            orderBy: {
                name: 'asc'
            }
        ...
        const dictCounts =
            categories.length > 0
                ? await this.prisma.dict.groupBy({
                      by: ['category'],
                      where: {
                          category: {
                ...
                : [];
        const countIndex = new Map(dictCounts.map((group) => [group.category, group._count._all]));

        return categories.map((category) => {
            return {
                label: category.name,
                value: category.name,
                count: countIndex.get(category.name) ?? 0
            };
        });
    }
    ...
    async createDict(data: CreateDictDto) {
        ...
        await this.assertDictCategoryExists(data.category);
    ...
    private async ensureDictCategoryAvailable(name: string) {
        const existingCategory = await this.prisma.dictCategory.findUnique({
            where: {
                name
            },
            select: {
    ...
    /**
     * 字典主记录只允许引用显式分类，写路径不补齐分类。
     */
    private async assertDictCategoryExists(name: string) {
        const category = await this.prisma.dictCategory.findUnique({
            where: {
                name
            }
        });
