...
const mockRbacAssignmentService = {
    replaceRoleUsers: jest.fn(),
    replaceRoleGroups: jest.fn(),
    replaceRoleParentRoles: jest.fn(),
...
describe('SystemRolesService', () => {
    ...
    beforeEach(() => {
        ...
        mockRbacAssignmentService.replaceRoleUsers.mockResolvedValue(null);
        mockRbacAssignmentService.replaceRoleGroups.mockResolvedValue(null);
        mockRbacAssignmentService.replaceRoleParentRoles.mockResolvedValue(null);
        mockRbacAssignmentService.replaceRolePermissions.mockResolvedValue(null);
    ...
    it('分配直接用户时，应委托 RBAC assignment 中间层替换关系', async () => {
        const role = createRole();
        mockPrismaService.rbacRole.findFirst.mockResolvedValue(role);
        mockPrismaService.betterAuthUser.count.mockResolvedValue(1);
        mockPrismaService.rbacUserRole.findMany.mockResolvedValue([{ userId: 'u1' }]);

        await service.assignDirectUsers(role.id, ['u2', 'u2'], 'operator_1');

        expect(mockRbacAssignmentService.replaceRoleUsers).toHaveBeenCalledWith(
            role.id,
            ['u2', 'u2'],
            'operator_1',
            undefined
        );
        expect(mockPrismaService.rbacUserRole.createMany).not.toHaveBeenCalled();
    });
