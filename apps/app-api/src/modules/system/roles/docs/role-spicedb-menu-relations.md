...
- `apps/app-api/src/modules/system/roles/roles.controller.ts`
- `apps/app-api/src/modules/system/roles/roles.service.ts`
- `apps/app-api/src/modules/system/roles/dto/role.dto.ts`
- `apps/app-api/src/modules/authz-permission/authz-permission.controller.ts`
...
- `apps/app-api/src/modules/spicedb-projection/base-relation-projection.service.ts`
- `spicedb/schema.zed`
...
- 当前没有 app-api 专用的 manager 全量同步脚本；运行时写路径由业务服务同步源表与 SpiceDB。新增全量重建能力时，应单独提供 app 侧脚本和验证流程。

## 8. 前端边界

当前仓库没有 app-api 专用的角色与权限管理前端页面。本文只描述 app 侧后端能力；菜单 Page/Catalog 入口和核心 manager relation 仍是两套不同的后端授权概念。
