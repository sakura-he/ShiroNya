...
import { Prisma, RbacRole, RbacStatus } from '@app/prisma-app/generated/client';
...
export class SystemRolesService {
     ...
     * 分页查询后台角色列表及其基础元数据。
     */
    async getAllRoles(query: QueryRoleListDto, actorId: string, context?: RbacServiceContext) {
        ...
        const [records, pagination] = await this.prismaService.rbacRole.findManyAndCount({
            ...
            where: {
                deletedAt: null,
                ...this.buildRoleListKeywordWhere(query.keyword ?? query.name),
        ...
        return {
            records: await this.attachViewerRoleCapabilities(records, actorId, context),
            meta: {
                viewerCanCreateRole: await checkRbacServicePermission(
                    this.rbacAuthorizationService,
                    actorId,
     ...
     */
    async getRoleRelations(id: number, actorId: string, context?: RbacServiceContext) {
        ...
        return {
            role: await this.attachViewerRoleCapability(role, actorId, context),
            directUserIds,
            userGroupIds,
            parentRoleIds,
            permissionIds,
     ...
     */
    async getRoleAssignableUsers(query: QueryRoleAssignableUserDto, actorId: string, context?: RbacServiceContext) {
        await this.assertRoleViewable(query.roleId, actorId, context);
     ...
     */
    async getRoleAssignableUserGroups(
        query: QueryRoleAssignableUserGroupDto,
        actorId: string,
     ...
     * 分页查询角色可继承的父角色。
     */
    async getRoleAssignableParentRoles(
        query: QueryRoleAssignableParentDto,
        actorId: string,
        context?: RbacServiceContext
    ) {
        await this.assertRoleViewable(query.roleId, actorId, context);
     ...
     * 分页查询角色可授予权限。
     */
    async getRoleAssignablePermissions(
        query: QueryRoleAssignablePermissionDto,
        actorId: string,
        context?: RbacServiceContext
    ) {
        await this.assertRoleViewable(query.roleId, actorId, context);
     ...
     */
    async getRoleEffectiveUsers(query: QueryRoleEffectiveUserDto, actorId: string, context?: RbacServiceContext) {
        await this.assertRoleViewable(query.roleId, actorId, context);
     ...
     * 创建 RBAC 角色。
     */
    async createRole(createRoleDto: CreateRoleDto, actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(
            this.rbacAuthorizationService,
            actorId,
            RBAC_PERMISSIONS.SYSTEM_ROLE_CREATE,
        ...
        const result = await this.prismaService.rbacRole.create({
            data: {
                ...
                isSuperAdmin: createRoleDto.isSuperAdmin ?? false,
     ...
     * 更新指定 RBAC 角色元数据。
     */
    async updateRole(id: number, updateRoleDto: UpdateRoleDto, actorId: string, context?: RbacServiceContext) {
        ...
        const nextIsSuperAdmin =
            updateRoleDto.isSuperAdmin !== undefined ? updateRoleDto.isSuperAdmin : existingRole.isSuperAdmin;
        ...
        const result = await this.prismaService.rbacRole.update({
            ...
            data: {
                ...
                isSuperAdmin: updateRoleDto.isSuperAdmin,
    ...
    /**
     * 从角色视角替换直接用户分配关系，并刷新受影响用户状态版本。
     */
    async assignDirectUsers(id: RbacRole['id'], userIds: string[], actorId: string, context?: RbacServiceContext) {
        return await this.rbacAssignmentService.replaceRoleUsers(id, userIds, actorId, context);
    }

    /**
     * 从角色视角替换用户组分配关系，并刷新受影响用户和角色状态版本。
     */
    async assignUserGroups(id: RbacRole['id'], userGroupIds: number[], actorId: string, context?: RbacServiceContext) {
        return await this.rbacAssignmentService.replaceRoleGroups(id, userGroupIds, actorId, context);
    }

    async assignParentRoles(
        id: RbacRole['id'],
        parentRoleIds: number[],
        actorId: string,
        context?: RbacServiceContext
    ) {
        return await this.rbacAssignmentService.replaceRoleParentRoles(id, parentRoleIds, actorId, context);
    }
    ...
    async assignPermissions(
        ...
        return await this.rbacAssignmentService.replaceRolePermissions(id, permissionIds, actorId, context);
    }

    /**
     * 删除角色前先校验是否仍被后台用户引用。
     */
    async deleteRole(id: number, actorId: string, context?: RbacServiceContext) {
        const role = await this.findOne(id);
        if (!role) {
            throw new BusinessException(ErrorCodes.ROLE.NOT_FOUND);
        }
     ...
     */
    private async assertRoleViewable(id: number, actorId: string, context?: RbacServiceContext) {
        const role = await this.findOne(id);
        if (!role) {
            throw new BusinessException(ErrorCodes.ROLE.NOT_FOUND);
        }
    ...
    private buildRoleListKeywordWhere(keyword?: string): Prisma.RbacRoleWhereInput {
        return this.buildRoleKeywordWhere(keyword);
    }
     ...
     */
    private async attachViewerRoleCapabilities(records: RbacRole[], actorId: string, context?: RbacServiceContext) {
        if (records.length === 0) {
            return records.map((record) => this.withViewerRoleCapabilities(record));
        }

        const permissions = await this.getViewerRolePermissions(actorId, context);

        return records.map((record) => this.withViewerRoleCapabilities(record, permissions));
    }

    /**
     * 为单个角色记录补充当前用户的操作能力，缺失权限统一视为不可操作。
     */
    private async attachViewerRoleCapability(record: RbacRole, actorId: string, context?: RbacServiceContext) {
        return this.withViewerRoleCapabilities(record, await this.getViewerRolePermissions(actorId, context));
    }
    ...
    private async getViewerRolePermissions(
        actorId: string,
        context?: RbacServiceContext
