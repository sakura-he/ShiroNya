...
import { Prisma, RbacPermissionGroup } from '@app/prisma-app/generated/client';
...
export class SystemRbacPermissionGroupsService {
    ...
    async getGroupList(query: QueryRbacPermissionGroupListDto, actorId: string, context?: RbacServiceContext) {
    ...
    async createGroup(data: CreateRbacPermissionGroupDto, actorId: string, context?: RbacServiceContext) {
    ...
    async updateGroup(id: number, data: UpdateRbacPermissionGroupDto, actorId: string, context?: RbacServiceContext) {
    ...
    async deleteGroup(id: number, actorId: string, context?: RbacServiceContext) {
    ...
    async getGroupRelations(id: number, actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(this.authzService, actorId, RBAC_PERMISSIONS.PERMISSION_VIEW, context);
        const group = await this.getExistingGroup(id);
        ...
        return {
            group: this.withCapabilities(group, await this.getMeta(actorId, context)),
            permissionIds: permissionRows.map((row) => row.id),
            menuIds: menuRows.map((row) => row.id)
        };
    }

    async assignRelations(data: AssignRbacPermissionGroupRelationsDto, actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(this.authzService, actorId, RBAC_PERMISSIONS.PERMISSION_UPDATE, context);
        ...
        return await this.getGroupRelations(data.groupId, actorId, context);
    }
