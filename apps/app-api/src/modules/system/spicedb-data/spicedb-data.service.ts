...
const HEALTH_THRESHOLD_ENV = {
    lagWarn: 'APP_SPICEDB_HEALTH_LAG_WARN',
    dlqWarn: 'APP_SPICEDB_HEALTH_DLQ_WARN',
    driftWarn: 'APP_SPICEDB_HEALTH_DRIFT_WARN',
    checkErrorRateWarn: 'APP_SPICEDB_HEALTH_CHECK_ERROR_RATE_WARN'
} as const;
...
export class SpiceDbDataService {
    ...
    private async repairProjectionAfterRelationshipUpdates(
        ...
        const repairThreshold = this.getNumberConfig('APP_SPICEDB_TARGETED_REPAIR_THRESHOLD', 500);
        if (updates.length === 0) {
            return {
                repaired: true,
                patchedCount: 0
