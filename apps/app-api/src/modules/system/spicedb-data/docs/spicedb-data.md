...
- `apps/app-api/src/modules/system/spicedb-data/spicedb-data.module.ts`
- `apps/app-api/src/modules/system/spicedb-data/spicedb-data.controller.ts`
- `apps/app-api/src/modules/system/spicedb-data/spicedb-data.service.ts`
- `apps/app-api/src/modules/system/spicedb-data/spicedb-projection-reconcile.service.ts`
- `apps/app-api/src/modules/system/spicedb-data/dto/spicedb-data.dto.ts`

## 2. 一句话总览

`SpiceDbDataService` 是 app-api 管理侧 SpiceDB 运维入口，提供 schema、relationship、permission check、投影同步概况和对账历史，方便直观看当前 SpiceDB 状态。它不负责 app 侧基础菜单、角色、用户组或按钮授权；这些由 RBAC 源表和 effective 读模型决定。
