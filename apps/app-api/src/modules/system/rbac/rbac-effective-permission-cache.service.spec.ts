import { APP_API_RBAC_CACHE_NAMESPACE, createRbacEffectivePermissionCodesCacheKey } from '@app/common/constants';
...
describe('SystemRbacEffectivePermissionCacheService', () => {
    ...
    it('按 userId 和 userVersion 读取 effective permission codes', async () => {
        ...
        expect(cache.get).toHaveBeenCalledWith(
            createRbacEffectivePermissionCodesCacheKey(APP_API_RBAC_CACHE_NAMESPACE, 'user:1', 'user_v1')
        );
    });
    ...
    it('写入 effective permission codes 时使用毫秒 TTL', async () => {
        ...
        expect(cache.set).toHaveBeenCalledWith(
            createRbacEffectivePermissionCodesCacheKey(APP_API_RBAC_CACHE_NAMESPACE, 'u1', 'user_v1'),
            {
