...
本文说明 `apps/app-api/src/modules/system/rbac` 当前 RBAC 设计。app-api 后台管理侧基础接口授权只走本地 RBAC。Guard、AuthorizationService、请求缓存、effective permission cache 基类和图计算位于 `libs/rbac-core`；本模块负责 `rbac_*` Prisma 适配、effective 表写入、错误码、可信控制面上下文和管理接口。
...
| app adapter      | `apps/app-api/src/modules/system/rbac/*`       | 注入 `rbac_*` Prisma model、`app-api` cache namespace、错误码、user-state 版本、可信控制面边界和管理接口。 |

事实来源是 `rbac_*` 源表和 `rbac_effective_*` 读模型；`rbac-core` 不直接访问数据库。metadata key 和 request cache 直接从 `@app/rbac-core` 引入。

## 2. 新手快速路径

新加一个需要 RBAC 保护的 app-api 管理侧接口：

1. 在 `rbac-permissions.ts` 的 `RBAC_PERMISSIONS` 中声明稳定 code。
2. 在 controller handler 上写 `@RbacPermission(RBAC_PERMISSIONS.X)`。
3. 如果 service 方法可能被任务、RPC/gRPC、其他 controller 复用，service 内再调用 `assertRbacServicePermission(...)` 或 `RbacAuthorizationService.assertPermission(...)`。
...
```mermaid
flowchart TD
    A["HTTP 请求进入 app-api"] --> B["BetterAuthSessionGuard"]
...
- `RbacAuthorizationService`：本地 DI 入口，实际判断顺序在 `RbacAuthorizationServiceBase`，Prisma 读取由 `AppRbacAuthorizationStore` 实现。
...
| 场景                                                | 推荐写法                                                                   | 说明                                                                        |
| --------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| 普通 app-api 管理侧 HTTP 业务接口                  | `@RbacPermission(RBAC_PERMISSIONS.X)`                                      | 推荐入口；写运行时权限 code，也进入权限发现树。                             |
...
| service 内部敏感动作、复用方法、任务、RPC/gRPC      | `assertRbacServicePermission(...)` 或 `assertPermission(...)`              | 不依赖路由 metadata，由业务代码显式触发最终断言。                           |
| admin-api 可信控制面已经完成授权后调用 app 侧 RPC | `RbacServiceContext.authorizationBoundary = 'trusted-admin-control-plane'` | 只用于经过控制面身份校验的内部调用。                                        |

RBAC 当前入口是 `@RbacPermission()`、`@RbacDeclarePermissions()`、`RbacGuard`、`RbacAuthorizationService` 和 app-api 的 `rbac-service-context` 辅助函数；新代码使用这些入口。
...
app-api 的 RBAC 管理 service 通常使用 `rbac-service-context.ts` 里的辅助函数：
...
export class SystemMenusService {
    ...
    async deleteMenu(id: number, actorId: string, context?: RbacServiceContext) {
        ...
        return await this.deleteMenuRecord(id);
    }
}
```
...
| 方法                          | 返回/行为                                      | 适合场景                                            |
| ----------------------------- | ---------------------------------------------- | --------------------------------------------------- |
| `assertRbacServicePermission` | 无权限直接抛业务异常；可信控制面可跳过重复判权 | app-api 管理 service 的最终裁决入口。              |
| `checkRbacServicePermission`  | 返回 `boolean`                                 | 带 `RbacServiceContext` 的单个能力分支。            |
| `checkRbacServicePermissions` | 返回 `Map<string, boolean>`                    | 详情页 `viewerCan`、批量按钮态、能力提示。          |
| `assertPermission`            | 无权限直接抛业务异常                           | 不需要控制面上下文时直接调用 RBAC 授权服务。        |
| `checkPermissions`            | 返回 `Map<string, boolean>`                    | 不需要控制面上下文时做批量能力查询。                |
| `getGrantedCodes`             | 返回 effective code 数组                       | `/account/navigation`、前端展示能力，不做最终裁决。 |

`trusted-admin-control-plane` 只能用于已经通过 admin-api 控制面身份、scope、审计边界校验的内部调用，不能用于普通 HTTP 用户入口。
...
| `rbac-permission.decorator.ts`                                          | 使用 core factory + app discovery 装饰器生成 `@RbacPermission()`。                    |
...
| `rbac-authorization.service.ts`                                         | 继承 `RbacAuthorizationServiceBase`，实现 `AppRbacAuthorizationStore`。               |
| `rbac-effective-permission-cache.service.ts`                            | 继承 core cache base，设置 namespace 为 `app-api`。                                   |
| `rbac-service-context.ts`                                               | app-api service 层可信控制面边界辅助。                                                |
...
pnpm exec jest apps/app-api/src/modules/system/rbac/rbac.guard.spec.ts apps/app-api/src/modules/system/rbac/rbac-authorization.service.spec.ts --runInBand
pnpm exec jest apps/app-api/src/modules/system/rbac/rbac-effective-permission-cache.service.spec.ts apps/app-api/src/modules/system/rbac/rbac-graph.service.spec.ts --runInBand
pnpm exec jest apps/app-api/src/modules/system/discovery/discovery.service.spec.ts apps/app-api/src/modules/spicedb/admin-spicedb-nestjs-resolvers.spec.ts --runInBand
pnpm build:app-api
```
