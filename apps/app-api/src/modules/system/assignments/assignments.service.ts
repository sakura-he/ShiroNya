...
import { assertRbacServicePermission, type RbacServiceContext } from '../rbac/rbac-service-context';
...
export class SystemRbacAssignmentsService {
    ...
    async replaceUserRoles(userId: string, roleIds: number[], actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(this.authzService, actorId, RBAC_PERMISSIONS.USER_ASSIGN_ROLE, context);
    ...
    async replaceUserGroups(userId: string, groupIds: number[], actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(this.authzService, actorId, RBAC_PERMISSIONS.USER_ASSIGN_USER_GROUP, context);
        await this.ensureUserExists(userId);
    ...
    async replaceGroupMembers(groupId: number, userIds: string[], actorId: string, context?: RbacServiceContext) {
    ...
    async replaceGroupRoles(groupId: number, roleIds: number[], actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(
            this.authzService,
            actorId,
    ...
    async replaceRoleUsers(roleId: number, userIds: string[], actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(
            this.authzService,
            actorId,
    ...
    async replaceRoleGroups(roleId: number, groupIds: number[], actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(
            this.authzService,
            actorId,
    ...
    async replaceRolePermissions(
        ...
        await assertRbacServicePermission(this.authzService, actorId, RBAC_PERMISSIONS.ROLE_ASSIGN_PERMISSION, context);
        const role = await this.ensureRoleExists(roleId);
    ...
    async replacePermissionRoles(
        ...
        await this.prismaService.$transaction(async (tx) => {
            await tx.rbacRolePermission.deleteMany({ where: { permissionId } });
            const uniqueRoleIds = this.normalizeNumberIds(roleIds);
            if (uniqueRoleIds.length > 0) {
                await tx.rbacRolePermission.createMany({
                    data: uniqueRoleIds.map((roleId) => ({
        ...
        const affectedRoleIds = [...previousRoleIds, ...roleIds];
        await Promise.all([
            this.rebuildAffectedUsers(await this.graphService.getAffectedUserIdsByRoleIds(affectedRoleIds)),
            this.bumpRoleStateVersions(affectedRoleIds)
        ]);
    ...
    async replaceMenuViewerRoles(menuId: number, roleIds: number[], actorId: string, context?: RbacServiceContext) {
        await assertRbacServicePermission(
            this.authzService,
            actorId,
