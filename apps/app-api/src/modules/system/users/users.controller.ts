...
export class SystemUsersController {
    ...
    constructor(private readonly userService: SystemUsersService) {}
    ...
    async assignRoles(@Session() session: BetterAuthSession, @Body() data: AssignUserRolesDto) {
        await this.userService.assignRoles(data.userId, data.roleIds, session.user.id);
        return null;
    }
    ...
    async assignUserGroups(@Session() session: BetterAuthSession, @Body() data: AssignUserGroupsDto) {
        await this.userService.assignUserGroups(data.userId, data.groupIds, session.user.id);
        return null;
    }
}
