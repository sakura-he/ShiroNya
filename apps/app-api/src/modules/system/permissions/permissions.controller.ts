...
export class SystemRbacPermissionsController {
    constructor(private readonly permissionService: SystemRbacPermissionsService) {}
    ...
    async assignRoles(@Session() session: BetterAuthSession, @Body() data: AssignRbacPermissionRolesDto) {
        return await this.permissionService.assignRoles(data.permissionId, data.roleIds, session.user.id);
    }
}
