...
const ADMIN_DEVTOOLS_REQUEST_HEADER = 'x-admin-api-devtools';
const ADMIN_DEVTOOLS_ENABLED_HEADER = 'x-admin-api-devtools-enabled';
...
export class ResponseFormatInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        if (request.path === '/metrics' || request.path === '/admin/metrics') {
    ...
    private attachDevtoolsModeHeader(response: Response): void {
        response.setHeader?.(ADMIN_DEVTOOLS_ENABLED_HEADER, isAdminApiDevtoolsDebugEnabled() ? '1' : '0');
    }
