...
describe('ResponseFormatInterceptor', () => {
    ...
    const originalAdminApiDevtoolsDebug = process.env.ADMIN_API_DEVTOOLS_DEBUG;
    ...
    afterEach(() => {
        ...
        if (originalAdminApiDevtoolsDebug === undefined) {
            delete process.env.ADMIN_API_DEVTOOLS_DEBUG;
        } else {
            process.env.ADMIN_API_DEVTOOLS_DEBUG = originalAdminApiDevtoolsDebug;
        }
    });
    ...
    it.each(['/metrics', '/admin/metrics'])('%s 应保持原始响应且不写用户状态头', async (path) => {
        const adminUserStateService = {
            attachUserStateHeaders: jest.fn(async () => undefined)
        };
        const interceptor = new ResponseFormatInterceptor(adminUserStateService as any);
    ...
    it('开发态且 devtools header 开启时，应返回 http 和 SpiceDB debug 信息', async () => {
        ...
        process.env.ADMIN_API_DEVTOOLS_DEBUG = 'true';
        ...
        expect(response.setHeader).toHaveBeenCalledWith('x-admin-api-devtools-enabled', '1');
    });
    ...
    it('未开启服务端 devtools debug 开关时，即使带 header 也不应返回 debug', async () => {
        ...
        expect(response.setHeader).toHaveBeenCalledWith('x-admin-api-devtools-enabled', '0');
    });

    it('服务端 devtools debug 开关开启时，应通过响应头通知前端开启下一次 SpiceDB debug 请求', async () => {
        process.env.NODE_ENV = 'development';
        process.env.ADMIN_API_DEVTOOLS_DEBUG = '1';
        const adminUserStateService = {
            attachUserStateHeaders: jest.fn(async () => undefined)
        };
        const interceptor = new ResponseFormatInterceptor(adminUserStateService as any);
