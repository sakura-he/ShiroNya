...
import { Prisma, RbacStatus } from '@app/prisma-admin/generated/client';
...
export class BaseRelationProjectionService {
    ...
    private async getEnabledRoleIndex(roleIds: number[]): Promise<Map<number, { id: number; code: string }>> {
        const uniqueRoleIds = this.uniqueNumbers(roleIds);
        if (uniqueRoleIds.length === 0) {
            return new Map();
        }
