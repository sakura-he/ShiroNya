...
import { createAdminAuthzProjectionCacheRedisKey } from '@app/common/constants';
...
export class ProjectionAuthzReadService {
    ...
    private async checkTaskPermissionsFromProjection(
        ...
        const [tasks, managerPermissions, objectBindings] = await Promise.all([
            this.prismaService.task.findMany({
                where: {
                    id: {
                        in: taskIds
            ...
            Promise.all(
                permissions.map(
                    async (permission) =>
                        [permission, await this.hasTaskObjectManagerPermission(permission, roleIndex)] as const
                )
            ),
            this.prismaService.authzObjectSubjectBinding.findMany({
                where: {
        ...
        const managerPermissionIndex = new Map<TaskPermission, boolean>(managerPermissions);
        ...
        return taskIds.map((taskId) => {
            ...
            for (const permission of permissions) {
                permissionsByTask[permission] =
                    isSuperAdmin ||
                    managerPermissionIndex.get(permission) === true ||
    ...
    private isSuperAdmin(roleIndex: Map<number, RoleIndexRow>): boolean {
        return [...roleIndex.values()].some((role) => role.code === 'super_admin');
    }
    ...
    private async createCacheKey(scope: string, parts: string[]): Promise<string> {
        ...
        return createAdminAuthzProjectionCacheRedisKey(scope, version, parts);
    }
