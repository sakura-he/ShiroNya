import {
    ...
    ADMIN_ROLE_VERSION_REDIS_KEY_PREFIX,
    ADMIN_USER_STATE_REDIS_KEY_PREFIX,
...
const ADMIN_PREFIX = ADMIN_USER_STATE_REDIS_KEY_PREFIX;
const MENU_CACHE_KEY = ADMIN_MENU_VERSION_REDIS_KEY;
const ROLE_CACHE_KEY_PREFIX = ADMIN_ROLE_VERSION_REDIS_KEY_PREFIX;
...
describe('AdminUserStateService', () => {
    describe('属性测试', () => {
        ...
        // Feature: admin-user-version, Property 4: 所有 Redis 键以 ver:admin: 开头
        it('性质 4：所有 Redis 键都应以 ver:admin: 开头', async () => {
            const fixture = await createServiceFixture();
                    ...
                    fc.asyncProperty(userIdArbitrary, roleIdArbitrary, async (userId, roleId) => {
                        ...
                        expect(touchedKeys).toEqual(
                            expect.arrayContaining([
                                createAdminUserVersionRedisKey(userId),
                                createAdminRoleVersionRedisKey(roleId),
                                MENU_CACHE_KEY
                            ])
                        );
    ...
    describe('单元测试', () => {
        ...
        it('菜单版本应使用 admin_global，而不是 global', async () => {
            const fixture = await createServiceFixture({
                initialDbRecords: [
                    {
                        type: StateVersionType.menu,
        ...
        it('同一 request 内重复写入响应头时应复用综合版本计算', async () => {
            const fixture = await createServiceFixture({
                initialRedisEntries: {
                    ...
                    [createAdminUserVersionRedisKey('user_1')]: 'user-version',
