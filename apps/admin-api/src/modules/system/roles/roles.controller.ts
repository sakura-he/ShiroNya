...
export class SystemRolesController {
    ...
    async assignUsers(@Session() session: BetterAuthSession, @Body() data: AssignRoleUserDto) {
        await this.rbacAssignmentService.replaceRoleUsers(data.roleId, data.userIds, session.user.id);
        return null;
    }
    ...
    async assignUserGroups(@Session() session: BetterAuthSession, @Body() data: AssignRoleUserGroupDto) {
        await this.rbacAssignmentService.replaceRoleGroups(data.roleId, data.userGroupIds, session.user.id);
        return null;
    }
    ...
    async assignParentRoles(@Session() session: BetterAuthSession, @Body() data: AssignRoleParentDto) {
        await this.rbacAssignmentService.replaceRoleParentRoles(data.roleId, data.parentRoleIds, session.user.id);
        return null;
    }
    ...
    async assignPermissions(@Session() session: BetterAuthSession, @Body() data: AssignRolePermissionDto) {
        await this.rbacAssignmentService.replaceRolePermissions(data.roleId, data.permissionIds, session.user.id);
        return null;
    }
}
