...
- `apps/admin-api/src/modules/system/users/users.controller.ts`
- `apps/admin-api/src/modules/system/users/users.service.ts`
- `apps/admin-api/src/modules/system/users/dto/user.dto.ts`
- `apps/admin-api/src/modules/better-auth/admin-user-admin.service.ts`
- `apps/admin-api/src/modules/system/rbac/rbac-graph.service.ts`
- `prisma/admin/rbac.prisma`

## 2. 一句话总览

`SystemUsersService` 通过 Better Auth 管理后台用户主表，通过业务库维护 profile；角色、用户组、有效角色和可见菜单关系都读取 RBAC 源表或 effective 读模型。用户直接角色分配由角色页和 RBAC 用户页关系管理维护。
...
- `profile` 相关字段：来自 `shiro_better_auth_user_profile`。
- Better Auth 主表字段：来自 `shiro_better_auth_user`。
...
- SpiceDB 用于对象关系权限；admin 用户基础角色、用户组和菜单授权由 RBAC 源表与 effective 读模型处理。
