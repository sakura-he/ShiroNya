...
import { MenuStatusEnum, MenuTypeEnum, Prisma, RbacStatus } from '@app/prisma-admin/generated/client';
...
export class SystemUserGroupsService {
     ...
     */
    async getUserGroupList(query: QueryUserGroupListDto, actorId: string) {
        await this.rbacAuthorizationService.assertPermission(actorId, RBAC_PERMISSIONS.SYSTEM_USER_GROUP_VIEW);
        ...
        const recordsWithCapabilities = await this.attachViewerUserGroupCapabilities(recordsWithRelations, actorId);

        return {
            records: recordsWithCapabilities,
            meta: {
                viewerCanCreateUserGroup: await this.rbacAuthorizationService.checkPermission(
                    actorId,
     ...
     */
    async getUserGroupByIdForViewer(id: number, actorId: string) {
        await this.rbacAuthorizationService.assertPermission(actorId, RBAC_PERMISSIONS.SYSTEM_USER_GROUP_VIEW);
     ...
     */
    private async attachViewerUserGroupCapabilities(records: UserGroupView[], actorId: string) {
        if (records.length === 0) {
            return records.map((record) => this.withViewerUserGroupCapabilities(record));
        }

        const permissions = await this.getViewerUserGroupPermissions(actorId);
    ...
    private async getViewerUserGroupPermissions(actorId: string): Promise<Record<string, boolean>> {
     ...
     */
    async getUserGroupRelations(id: number, actorId: string) {
        const group = await this.getUserGroupByIdForViewer(id, actorId);
        const visibleMenuIds = await this.rbacGraphService.getGroupVisibleMenuIds(id);

        return {
            group: this.withViewerUserGroupCapabilities(group, await this.getViewerUserGroupPermissions(actorId)),
     ...
     */
    async getUserGroupRelationMembers(query: QueryUserGroupRelationMemberDto, actorId: string) {
        await this.getUserGroupByIdForViewer(query.groupId, actorId);
     ...
     */
    async getUserGroupRelationRoles(query: QueryUserGroupRelationRoleDto, actorId: string) {
        await this.getUserGroupByIdForViewer(query.groupId, actorId);
     ...
     */
    async getUserGroupRelationMenus(query: QueryUserGroupRelationMenuDto, actorId: string) {
        await this.getUserGroupByIdForViewer(query.groupId, actorId);
     ...
     * 创建 RBAC 用户组元数据。
     */
    async createUserGroup(operatorUserId: string, data: CreateUserGroupDto) {
        await this.rbacAuthorizationService.assertPermission(operatorUserId, RBAC_PERMISSIONS.SYSTEM_USER_GROUP_CREATE);
     ...
     */
    async updateUserGroup(id: number, data: UpdateUserGroupDto, actorId: string) {
        await this.rbacAuthorizationService.assertPermission(actorId, RBAC_PERMISSIONS.SYSTEM_USER_GROUP_UPDATE);
     ...
     */
    async assignMembers(groupId: number, userIds: string[], actorId: string) {
        await this.rbacAssignmentService.replaceGroupMembers(groupId, userIds, actorId);
        return await this.getUserGroupRelations(groupId, actorId);
    }

    /**
     * 替换用户组继承角色集合，并刷新用户组成员和受影响角色的状态版本。
     */
    async assignRoles(groupId: number, roleIds: number[], actorId: string) {
        await this.rbacAssignmentService.replaceGroupRoles(groupId, roleIds, actorId);
        return await this.getUserGroupRelations(groupId, actorId);
    }
     ...
     */
    async deleteUserGroup(id: number, actorId: string) {
        await this.rbacAuthorizationService.assertPermission(actorId, RBAC_PERMISSIONS.SYSTEM_USER_GROUP_DELETE);
