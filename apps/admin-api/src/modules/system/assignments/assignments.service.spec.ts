...
describe('SystemRbacAssignmentsService', () => {
    ...
    beforeEach(() => {
        ...
        mockPrismaService.rbacRole.findMany.mockImplementation(({ where }) => {
            const roleIds = (where.id.in as number[]) ?? [];
            if (where.isSuperAdmin === true) {
                return [];
            }
            return roleIds.map((id) => ({ id, name: `role_${id}` }));
        });
    ...
    it('replaceRolePermissions 和 replacePermissionRoles 统一维护角色权限关系', async () => {
        ...
        expect(mockGraphService.getAffectedUserIdsByRoleIds).toHaveBeenCalledWith([3, 4]);
    });

    it('replaceRolePermissions 拒绝修改超级管理员角色的直接权限', async () => {
        mockPrismaService.rbacRole.findFirst.mockResolvedValueOnce({
            id: 1,
            name: 'rbac_super_admin',
            isSuperAdmin: true
        });

        await expect(service.replaceRolePermissions(1, [100], 'operator')).rejects.toBeInstanceOf(Error);

        expect(mockPrismaService.rbacPermission.count).not.toHaveBeenCalled();
        expect(mockPrismaService.rbacRolePermission.deleteMany).not.toHaveBeenCalled();
        expect(mockPrismaService.rbacRolePermission.createMany).not.toHaveBeenCalled();
        expect(mockGraphService.applyRebuild).not.toHaveBeenCalled();
        expect(mockAdminUserStateService.bumpRoleStateVersion).not.toHaveBeenCalled();
    });

    it('replacePermissionRoles 保留已有超级管理员角色的直接权限', async () => {
        mockPrismaService.rbacRolePermission.findMany.mockResolvedValueOnce([{ roleId: 1 }, { roleId: 2 }]);
        mockPrismaService.rbacRole.findMany.mockImplementation(({ where }) => {
            const roleIds = (where.id.in as number[]) ?? [];
            if (where.isSuperAdmin === true) {
                return roleIds.filter((id) => id === 1).map((id) => ({ id }));
            }
            return roleIds.map((id) => ({ id, name: `role_${id}` }));
        });

        await service.replacePermissionRoles(100, [1, 3, 3], 'operator');

        expect(mockPrismaService.rbacRolePermission.deleteMany).toHaveBeenCalledWith({
            where: { permissionId: 100, roleId: { notIn: [1] } }
        });
        expect(mockPrismaService.rbacRolePermission.createMany).toHaveBeenCalledWith({
            data: [{ roleId: 3, permissionId: 100, createdBy: 'operator' }],
            skipDuplicates: true
        });
        expect(mockGraphService.getAffectedUserIdsByRoleIds).toHaveBeenCalledWith([2, 3]);
    });
    ...
    it('replaceMenuViewerRoles 通过菜单 requiredPermissionCode 替换拥有该菜单权限的角色集合', async () => {
        ...
        expect(mockGraphService.getAffectedUserIdsByRoleIds).toHaveBeenCalledWith([1, 2]);
    });
});
