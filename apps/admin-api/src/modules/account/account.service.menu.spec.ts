import { createAdminAccountNavigationRedisKey } from '@app/common/constants';
...
describe('AccountService.getAccountNavigation', () => {
    ...
    it('导航缓存缺少父级菜单时应丢弃缓存并重新构建', async () => {
        ...
        expect(mockRedis.del).toHaveBeenCalledWith(createAdminAccountNavigationRedisKey('user_1', 'state_v1'));
        expect(result.menus.map((item) => item.id)).toEqual([151, 79]);
    });
    ...
    it('组合导航命中版本化缓存时不应再次访问 RBAC 读模型', async () => {
        mockRedis.get.mockResolvedValue(
            JSON.stringify({
                ...
                permissions: ['system.user.view', 'system.dict.create', 'rbac.test.page']
            })
        );

        const result = await service.getAccountNavigation(mockSession);

        expect(mockRedis.get).toHaveBeenCalledWith(createAdminAccountNavigationRedisKey('user_1', 'state_v1'));
        expect(mockRbacGraphService.getUserEffectiveState).not.toHaveBeenCalled();
        expect(mockRbacAuthorizationService.getGrantedCodes).not.toHaveBeenCalled();
        expect(result.permissions).toEqual(['system.user.view', 'system.dict.create', 'rbac.test.page']);
        expect(result.userStateVersion).toBe('state_v1');
    });
});
